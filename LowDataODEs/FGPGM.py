...
class FGPGM(object):
    ...
    def getFGPGMResults(self, GPPosteriorInit=True, blockNegStates=False,
                        ...
                        nSamples=100000, nBurnin=5000, propStds=None, ret_samples=False):
        """
        ...
        if GPPosteriorInit:
            ...
            for i in np.arange(len(self.kernels)):
                ...
                currentY = stdY[:, i]
                print(currentY.shape, currentCPhi.shape, currentSigma.shape)
        ...
        if ret_samples:
            idx = np.random.choice(MCMCSamples.shape[0], size = ret_samples, replace=False)
        ...
        stateAccepted = np.asarray(nAccepted[:-self.nODEParams], dtype=np.float64)
        stateRejected = nRejected[:-self.nODEParams]
        paramAccepted = np.asarray(nAccepted[-self.nODEParams:], dtype=np.float64)
        ...
        if ret_samples:
            # Only take the last subset of the samples
            subsetSamples = MCMCSamples[:ret_samples*5]
            idx = np.random.choice(subsetSamples.shape[0], size = ret_samples, replace=False)
            return  newStates, newParams, subsetSamples[idx, -self.nODEParams:]*(10**thetaMagnitudes),
